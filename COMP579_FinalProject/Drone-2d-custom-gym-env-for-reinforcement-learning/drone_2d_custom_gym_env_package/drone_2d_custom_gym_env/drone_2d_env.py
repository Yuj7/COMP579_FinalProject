...
class Drone2dEnv(gym.Env):
    ...
    def __init__(self, render_sim=False, render_path=True, render_shade=True, shade_distance=70,
                 ...
                 initial_force=5000, initial_rotation_force=600, step_penalty=0.0,
                 goal_reward=20.0, death_penalty=-10.0,success_radius=25.0,wind=None,wind_magnitude=100.0):
    ...
    def step(self, action):
        ...
        reward-=self.step_penalty
         #distance reward
        #reward+= np.exp(-1*(obs[4]**2+obs[5]**2)**(0.5)) #distance reward 
        if success:
            self.done = True
    ...
    def render(self, mode='human', close=False):
        ...
        directions = {0: "Downward", 1: "Upward", 2: "Toward Left", 3: "Toward Right"}

        if self.wind == "Uniform":
            # .get() provides a fallback if self.wind_dir isn't 0-3
            dir_name = directions.get(self.wind_dir, "Unknown")
            display_text = f"Wind Direction: {dir_name}"
        elif self.wind == "Random":
            display_text = "Wind: Random"
        else:
            display_text = "Wind: None"
    ...
    def reset(self):
        ...
        self.wind_dir=uniform_wind_dir
        #x, y = self.drone.frame_shape.body.position
        #target_x, target_y = self.x_target, self.y_target
        #self.prev_distance=np.sqrt((x-target_x)**2+(y-target_y)**2)
        self.reset_episode_state()
