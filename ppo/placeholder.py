...
#     def compute_return(self, rewards: list[float], last_val: float = 0.0) -> list[float]:
...
#                 if not done:
#                     with torch.no_grad():
#                         last_val = self.value(torch.tensor(state, dtype = torch.float32)).item()
                
#                 batch_returns.extend(self.compute_return(ep_rewards, last_val))
