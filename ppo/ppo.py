...
class PPO():
    ...
    def __init__(
        ...
        self.action_dim = self.training_env.action_space.n
        self.state_dim = self.training_env.observation_space.shape[0]
    ...
    def compute_return(rewards : list[float]) -> list[float]:
        returns = []
    ...
    def compute_advantage(self, returns : torch.Tensor, states : torch.Tensor) -> torch.Tensor:
    ...
    def collect_batch(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            while current_t < self.time_steps_per_batch:
                ...
                while not done:
                    action, log_prob = self.policy.get_action(torch.tensor(state))
                    action = action.numpy()
        ...
        batch_returns = torch.tensor(np.array(batch_returns))
        batch_states = torch.tensor(np.array(batch_states).T.squeeze(0))
        batch_actions = torch.tensor(np.array(batch_actions).T.squeeze(0))
        batch_log_probs = torch.tensor(np.array(batch_log_probs).T.squeeze(0))
    ...
    def learn(self):
        ...
        for _ in range(self.epochs):
            ...
            for states, actions, returns in zip(mini_batches_states, mini_batches_actions, mini_batches_returns):
                mini_batch_advantages = self.compute_advantage(returns, states)
                current_log_probs = self.get_log_prob(states, actions)
                ratios = torch.exp(current_log_probs)/torch.exp(mini_batches_log_probs)
    ...
    def split_into_mini_batch(
        ...
        if self.time_steps_per_batch % self.mini_batch_size == 0:
            mini_batches_sizes = [self.mini_batch_size for _ in range(self.time_steps_per_batch / self.mini_batch_size)]
            mini_batches = random_split(shuffled_batch_idx, mini_batches_sizes)
        else:
            mini_batches_sizes = [self.mini_batch_size for _ in range(self.time_steps_per_batch // self.mini_batch_size)]
            mini_batches_sizes.append(self.time_steps_per_batch - self.time_steps_per_batch // self.mini_batch_size * self.mini_batch_size)
            mini_batches = random_split(shuffled_batch_idx, mini_batches_sizes)
    ...
    def evaluation_phase(self):
        ...
        for ep in range(self.nb_eval_episodes):
            ...
            ep_rewards = 0
            while not done:
                action, log_prob = self.policy.get_action(torch.tensor(state), deterministic = True)
                action = action.numpy()
                log_prob = log_prob.numpy()
                next_state, reward, done, _ = self.training_env.step(action)
            ...
            eval_returns += 1/self.nb_eval_episodes * ep_rewards
